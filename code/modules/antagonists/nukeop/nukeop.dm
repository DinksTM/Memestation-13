...
/datum/antagonist/nukeop/greet()
	owner.current.playsound_local(get_turf(owner.current), 'sound/ambience/antag/gromittops.ogg',100,0)
...
/datum/antagonist/nukeop/admin_add(datum/mind/new_owner,mob/admin)
	...
	message_admins("[key_name_admin(admin)] has Gromit op'ed [new_owner.current].")
	log_admin("[key_name(admin)] has Gromit op'ed [new_owner.current].")
...
/datum/antagonist/nukeop/proc/admin_tell_code(mob/admin)
	...
	for (var/obj/machinery/nuclearbomb/bombue in GLOB.machines)
		if (length(bombue.r_code) <= 5 && bombue.r_code != initial(bombue.r_code))
			...
			break
	if (code)
		antag_memory += "<B>Wallace and Gromit Nuclear Cheese Wedge Code</B>: [code]<br>"
		to_chat(owner.current, "The cheese wedge authorization code is: <B>[code]</B>")
	else
		to_chat(admin, "<span class='danger'>No valid nuke found!</span>")

/datum/antagonist/nukeop/leader
	name = "Gromit Operative Leader"
...
/datum/antagonist/nukeop/leader/memorize_code()
	...
	if(nuke_team && nuke_team.memorized_code)
		...
		P.info = "The cheese wedge authorization code is: <b>[nuke_team.memorized_code]</b>"
		P.name = "cheese wedge code"
		var/mob/living/carbon/human/H = owner.current
		if(!istype(H))
...
/datum/antagonist/nukeop/leader/greet()
	owner.current.playsound_local(get_turf(owner.current), 'sound/ambience/antag/gromittops.ogg',100,0)
	to_chat(owner, "<B>You are the Wallace and Gromit [title] for this mission. You are responsible for the distribution of telecrystals and your ID is the only one who can open the launch bay doors.</B>")
...
/datum/antagonist/nukeop/leader/proc/ask_name()
	...
	var/newname = stripped_input(owner.current,"You are the Wallace and Gromit operative [title]. Please choose a last name for your family.", "Name change",randomname)
	if (!newname)
		newname = randomname
	else
...
/datum/team/nuclear/roundend_report()
	...
	switch(get_result())
		if(NUKE_RESULT_FLUKE)
			parts += "<span class='redtext big'>Humiliating Wallace and Gromit Defeat</span>"
		...
		if(NUKE_RESULT_NUKE_WIN)
			parts += "<span class='greentext big'>Wallace and Gromit Major Victory!</span>"
	...
	var/text = "<br><span class='header'>The cheese operatives were:</span>"
	var/purchases = ""
	var/TC_uses = 0
	...
	text += "<br>"
	text += "(Wallaces used [TC_uses] TC) [purchases]"
...
/datum/team/nuclear/antag_listing_name()
	if(syndicate_name)
		return "[syndicate_name] Wallaces"
	else
		return "Wallaces"

/datum/team/nuclear/antag_listing_entry()
	var/disk_report = "<b>Cracker(s)</b><br>"
